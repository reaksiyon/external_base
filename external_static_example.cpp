...
int main() {
    ...
    HWND hGameWindow = FindWindow(NULL, "Window Name (like Google Chrome)");
    if (hGameWindow == NULL) {
    ...
    char gameName[] = "GameName.exe"; //write process exe name
    DWORD gameBaseAddress = GetModuleBaseAddress(_T(gameName),pID);
    DWORD offsetGameToBaseAdress = 0x001F4FC0; //write address
    std::vector<DWORD> pointsOffsets{ 0x68,0x88,0x08,0x00,0x08,0xA0,0x90,0x10,0xE8 }; //pointer offsets change this too
